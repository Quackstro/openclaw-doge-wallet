...
import type { DogeInvoice } from "./types.js";
...
export class ExpiryManager {
   ...
   */
  async expirePending(): Promise<string[]> {
    ...
    if (expiredIds.length > 0) {
      this.log(
        "info",
        `doge-wallet: expired ${expiredIds.length} pending invoice(s)`,
      );
    }
  ...
  async cleanup(): Promise<string[]> {
    ...
    if (removedIds.length > 0) {
      this.log(
        "info",
        `doge-wallet: marked ${removedIds.length} old invoice(s) for cleanup`,
      );
    }
   ...
   */
  async cleanupExpiredInvoices(): Promise<ExpiryCleanupResult> {
  ...
  getExpirySummary(): {
    pending: number;
    expiredSoon: number; // expires in next 10 minutes
    totalExpired: number;
  } {
    const all = this.invoiceManager.listInvoices();
    const now = Date.now();
 ...
 */
export function createExpiryManager(
