...
export class PaymentVerifier {
  ...
  async verifyPayment(
    ...
    try {
      ...
      this.log(
        "warn",
        `doge-wallet: failed to fetch tx ${txid}: ${err.message ?? err}`,
      );
      return {
        valid: false,
        confirmations: 0,
    ...
    if (!opReturnMatch) {
      this.log(
        "warn",
        `doge-wallet: OP_RETURN mismatch or missing for invoice ${invoice.invoiceId}`,
      );
    ...
    this.log(
      "info",
      `doge-wallet: payment verified for invoice ${invoice.invoiceId} (tx: ${txid}, ${tx.confirmations} confirmations)`,
    );
   ...
   */
  private checkOpReturn(tx: Transaction, invoiceId: string): boolean {
    // Find OP_RETURN output
    const opReturnOutput = tx.outputs.find(
      (o) => o.scriptType === "OP_RETURN" || o.script?.startsWith("6a"),
    );
 ...
 */
export function createPaymentVerifier(config: PaymentVerifierConfig): PaymentVerifier {
