...
import type { DogeInvoice, PaymentCallback, CallbackResponse } from "./types.js";
...
export class CallbackSender {
  ...
  async sendPaymentCallback(
    ...
    if (!isValidCallbackUrl(invoice.callback.url)) {
      this.log(
        "warn",
        `doge-wallet: blocked callback to invalid/private URL: ${invoice.callback.url}`,
      );
      return {
        success: false,
        attempts: 0,
    ...
    for (let attempt = 1; attempt <= this.retries + 1; attempt++) {
      try {
        const result = await this.doSend(
          ...
          payload,
          invoice.callback.token,
        );

        this.log(
          "info",
          `doge-wallet: callback sent to ${invoice.callback.url} (attempt ${attempt}, status ${result.statusCode})`,
        );
        ...
        this.log(
          "warn",
          `doge-wallet: callback attempt ${attempt} failed: ${lastError}`,
        );
      }
    ...
    this.log(
      "error",
      `doge-wallet: all ${this.retries + 1} callback attempts failed for invoice ${invoice.invoiceId}`,
    );
  ...
  private async doSend(
    ...
    token?: string,
  ): Promise<{ success: boolean; response?: CallbackResponse; statusCode: number; error?: string }> {
    ...
    try {
      ...
      let responseBody: CallbackResponse | undefined;
      try {
        const text = await response.text();
        if (text) {
          responseBody = JSON.parse(text) as CallbackResponse;
        }
      } catch {
 ...
 */
export function createCallbackSender(config: CallbackConfig = {}): CallbackSender {
