...
export const MAINNET_DEFAULT_FEE_STRATEGY: "low" | "medium" | "high" = "medium";
 ...
 */
export const MAINNET_POLICY_DEFAULTS: PolicyConfig = {
  enabled: true,
  tiers: {
 ...
 */
export const TESTNET_POLICY_DEFAULTS: PolicyConfig = {
  enabled: true,
  tiers: {
...
export const MAINNET_UTXO_DEFAULTS: UtxoConfig = {
...
export const TESTNET_UTXO_DEFAULTS: UtxoConfig = {
 ...
 */
export function validateMainnetConfig(config: DogeWalletConfig): ConfigValidationResult {
  ...
  if (config.utxo.minConfirmations < MAINNET_MIN_CONFIRMATIONS) {
    warnings.push(
      `Low confirmation requirement (${config.utxo.minConfirmations}). ` +
  ...
  if (config.policy.limits.dailyMax > MAINNET_DAILY_LIMIT_WARNING) {
    warnings.push(
      `High daily limit (${config.policy.limits.dailyMax} DOGE). ` +
  ...
  if (!config.policy.enabled) {
    warnings.push(
      "Spending policy is DISABLED on mainnet. All sends will be auto-approved. " +
  ...
  if (microMax > 10) {
    warnings.push(
      `High micro tier threshold (${microMax} DOGE). ` +
  ...
  if (mediumApproval === "auto" || mediumApproval === "auto-logged") {
    ...
    if (mediumMax > 100) {
      warnings.push(
        `Medium tier (up to ${mediumMax} DOGE) uses auto-approval. ` +
  ...
  if (config.policy.limits.cooldownSeconds < 10) {
    warnings.push(
      `Low cooldown (${config.policy.limits.cooldownSeconds}s). ` +
  ...
  if (config.utxo.dustThreshold < MAINNET_DUST_THRESHOLD) {
    warnings.push(
      `Low dust threshold (${config.utxo.dustThreshold} koinu). ` +
  ...
  if (config.policy.allowlist.length > 10) {
    warnings.push(
      `Large allowlist (${config.policy.allowlist.length} addresses). ` +
  ...
  if (config.fees.strategy === "low") {
    warnings.push(
      "Using 'low' fee strategy on mainnet. " +
 ...
 */
export function getDefaultPolicy(network: "mainnet" | "testnet"): PolicyConfig {
 ...
 */
export function getDefaultUtxoConfig(network: "mainnet" | "testnet"): UtxoConfig {
 ...
 */
export function getDefaultFeeConfig(network: "mainnet" | "testnet"): FeesConfig {
 ...
 */
export function getMinConfirmations(network: "mainnet" | "testnet"): number {
 ...
 */
export function getDustThreshold(network: "mainnet" | "testnet"): number {
 ...
 */
export function applyMainnetSafetyDefaults(config: DogeWalletConfig): DogeWalletConfig {
  if (config.network !== "mainnet") {
    return config;
  }
 ...
 */
export function runMainnetPreflightChecks(config: DogeWalletConfig): PreflightCheckResult {
