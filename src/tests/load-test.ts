...
function generateMockUtxos(count: number, address: string = "DTestAddress123"): MockUTXO[] {
...
async function testUtxoManagerPerformance(): Promise<TestResult> {
  ...
  try {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      if (sorted.length === 0) throw new Error("Sort failed");
    }
    ...
    for (let i = 0; i < 100; i++) {
      ...
      if (confirmed + unconfirmed < 0) throw new Error("Balance calculation error");
    }
...
async function testCoinSelectionPerformance(): Promise<TestResult> {
...
async function testInvoicePerformance(): Promise<TestResult> {
  const testName = "Invoice Operations - 100+ Invoices";
  const start = performance.now();
  
  try {
    const invoiceCount = 200;
    const invoices: Map<string, { id: string; amount: number; description: string; createdAt: string }> = new Map();
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      lookupTimes.push(performance.now() - lookupStart);
      if (!found) throw new Error("Invoice lookup failed");
    }
    ...
    for (let i = 0; i < 100; i++) {
      ...
      if (list.length === undefined) throw new Error("List failed");
    }
...
async function testRateLimiterPerformance(): Promise<TestResult> {
...
async function testApiFailoverPerformance(): Promise<TestResult> {
  const testName = "API Failover - Simulated Load";
  const start = performance.now();
  
  try {
    // Simulate failover provider behavior
    ...
    async function simulateRequest(provider: MockProvider): Promise<{ success: boolean; latency: number }> {
    ...
    async function failoverRequest(): Promise<{ provider: string; success: boolean; latency: number }> {
      if (primary.healthy) {
    ...
    const results: Array<{ provider: string; success: boolean; latency: number }> = [];
    ...
    for (let i = 0; i < requestCount; i++) {
      ...
      if (i === 30) primary.healthy = false;
      if (i === 60) primary.healthy = true;
    }
...
async function testMemoryUsage(): Promise<TestResult> {
  ...
  try {
    ...
    // Allocate invoices
    const invoices = new Map<string, object>();
    for (let i = 0; i < 1000; i++) {
