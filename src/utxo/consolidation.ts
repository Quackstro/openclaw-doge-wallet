 ...
 */
export function getUtxoSummary(utxos: UTXO[]): UtxoSummary {
  let confirmed = 0;
  let unconfirmed = 0;
  let locked = 0;
  let dust = 0;
  ...
  for (const u of utxos) {
    if (u.locked) locked++;
    else if (u.confirmations >= 1) confirmed++;
    else unconfirmed++;

    if (u.amount < DUST_THRESHOLD) dust++;

    if (u.amount < SMALL_THRESHOLD) small++;
    else if (u.amount > LARGE_THRESHOLD) large++;
    else medium++;
  }
 ...
 */
function estimateConsolidationFee(inputCount: number): number {
 ...
 */
export function shouldConsolidate(utxos: UTXO[]): ConsolidationRecommendation {
