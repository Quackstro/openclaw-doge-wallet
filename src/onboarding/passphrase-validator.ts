...
import type { PassphraseStrength, PassphraseScore } from './types.js';
...
function calculateEntropy(passphrase: string): number {
  ...
  if (/[^a-zA-Z0-9]/.test(passphrase)) charsetSize += 32; // Special chars
 ...
 * - > 60 bits: strong (good)
 */
function entropyToScore(entropy: number): PassphraseScore {
 ...
 */
function containsCommonWords(passphrase: string): boolean {
 ...
 */
export function getStrengthDescription(strength: PassphraseStrength): string {
 ...
 */
export function getSuggestionTip(): string {
