...
import { OnboardingState, type OnboardingSession, type EncryptedMnemonic } from './types.js';
...
export class OnboardingStateManager {
   ...
   */
  async getSession(chatId: string): Promise<OnboardingSession | null> {
   ...
   */
  async startSession(chatId: string): Promise<OnboardingSession> {
    await this.loadStore();
  ...
  async updateSession(
    chatId: string,
    updates: Partial<Omit<OnboardingSession, 'chatId' | 'startedAt'>>
  ...
  async transitionTo(
    ...
    additionalUpdates?: Partial<OnboardingSession>
  ): Promise<OnboardingSession | null> {
    const updates = {
      ...additionalUpdates,
      state: newState,
   ...
   */
  async setTempMnemonic(chatId: string, mnemonic: string, passphraseHash: string): Promise<void> {
    // Derive encryption key from passphrase hash
    const salt = randomBytes(16);
    const key = await scryptAsync(passphraseHash, salt, 32) as Buffer;
   ...
   */
  async getTempMnemonic(chatId: string, passphraseHash?: string): Promise<string | null> {
    const session = await this.getSession(chatId);
    if (!session) return null;
   ...
   * eliminate in-memory risk.
   */
  async consumeTempMnemonic(chatId: string, passphraseHash?: string): Promise<string | null> {
   ...
   * eliminate in-memory risk.
   */
  async clearTempMnemonic(chatId: string): Promise<void> {
    const session = await this.getSession(chatId);
    if (!session) return;
   ...
   */
  async setPassphraseHash(chatId: string, passphrase: string): Promise<void> {
    const salt = randomBytes(16);
   ...
   */
  async verifyPassphrase(chatId: string, passphrase: string): Promise<boolean> {
    const session = await this.getSession(chatId);
    if (!session?.tempPassphraseHash) return false;
   ...
   */
  async getPassphraseHash(chatId: string): Promise<string | null> {
    const session = await this.getSession(chatId);
   ...
   */
  async setVerificationWords(chatId: string, indices: number[]): Promise<void> {
    await this.updateSession(chatId, { 
   ...
   */
  async incrementVerificationAttempts(chatId: string): Promise<number> {
    const session = await this.getSession(chatId);
   ...
   */
  async incrementTotalVerificationAttempts(chatId: string): Promise<number> {
    const session = await this.getSession(chatId);
  ...
  private async loadStore(): Promise<void> {
    ...
    try {
      ...
      this.store = JSON.parse(raw) as StateStore;
    } catch (err: unknown) {
