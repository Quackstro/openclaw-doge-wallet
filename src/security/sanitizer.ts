...
export function sanitizeReference(
  input: unknown,
  maxLength: number = MAX_REFERENCE_LENGTH,
): SanitizationResult<string> {
  if (typeof input !== "string") {
...
export function validateAmount(
  ...
  const {
    minAmount = MIN_AMOUNT_DOGE,
    maxAmount = MAX_AMOUNT_DOGE,
    network,
    warnThreshold,
  } = options;
 ...
 */
export function validateCallbackUrl(input: unknown): SanitizationResult<string> {
  if (typeof input !== "string") {
...
export function validateConfirmations(
  input: unknown,
  options: ConfirmationValidationOptions = {},
...
export function sanitizeMetadata(
  ...
  try {
    const sanitized = sanitizeObject(input as Record<string, unknown>, 0, maxDepth);
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (value === null || value === undefined) {
      ...
      if (Array.isArray(value)) {
        ...
        result[sanitizedKey] = sanitizeObject(value as Record<string, unknown>, depth + 1, maxDepth);
      }
