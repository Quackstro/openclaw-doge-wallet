...
import { RegistryWatcher, ServiceDirectory } from '../chain/registry-watcher.js';
...
import type { HTLCDetails } from '../htlc/types.js';
import { createHTLC } from '../htlc/script.js';
...
export class QPClient extends EventEmitter {
  private config: Required<
    Pick<OrchestratorConfig, 'address' | 'pubkey' | 'privkey' | 'provider' | 'getUtxos' | 'changeAddress'>
  > & OrchestratorConfig;
  ...
  async callService(
    ...
    try {
      ...
      const { messageId, sessionManager } = await this.sendRequest(
        handshake.sessionId,
        handshake.sessionKey,
        handshake.remoteInfo,
        request.payload,
        transport,
      );
      ...
      const delivery = await this.awaitDelivery(
        sessionManager,
        messageId,
        transport,
      ...
      let payment: { txId: string; htlcId?: string };
      ...
      let ratingTxId: string | undefined;
      ...
      const result: ServiceResult = {
        success: true,
        response: delivery.body as Buffer | Record<string, unknown>,
        providerAddress: provider.providerAddress,
        paymentTxId: payment.txId,
        ratingTxId,
   ...
   */
  async initiateHandshake(provider: ServiceListing): Promise<{
    sessionId: number;
    sessionKey: Buffer;
   ...
   */
  private async waitForHandshakeAck(
    sessionId: number,
    ourEphemeralPrivkey: Buffer,
    providerLongTermPubkey: Buffer,
  ): Promise<{
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      const messages = await scanAddress(
        ...
        this.config.address,
        10,
        { messageTypes: [QPMessageType.HANDSHAKE_ACK] },
      );

      for (const msg of messages) {
        try {
          const payload = msg.message.payload as HandshakeAckPayload;
    ...
    throw new Error(
      `Handshake timeout: no ACK received within ${this.config.handshakeTimeoutBlocks} blocks`,
    );
  }
  ...
  async sendRequest(
    ...
    transport: SideloadTransport,
  ): Promise<{ messageId: string; sessionManager: SessionManager }> {
    this.assertNotDestroyed();
  ...
  async awaitDelivery(
    ...
    messageId: string,
    transport: SideloadTransport,
    timeoutMs: number = DEFAULT_DELIVERY_TIMEOUT_MS,
  ): Promise<SideloadMessage> {
    this.assertNotDestroyed();
  ...
  async pay(params: {
    ...
    if (totalInput < totalNeeded) {
      throw new Error(
        `Insufficient funds: have ${totalInput} koinu, need ${totalNeeded}`,
      );
    }
