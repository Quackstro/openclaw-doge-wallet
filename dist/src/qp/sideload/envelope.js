 ...
 */
export function deriveIV(sessionKey, counter) {
...
/** Valid message types */
const VALID_TYPES = new Set(['request', 'response', 'chunk', 'done', 'error']);
/**
 ...
 */
export function encryptMessage(session, msg) {
 ...
 */
export function decryptMessage(session, envelope) {
    ...
    if (!envelope.iv.equals(expectedIV)) {
        throw new Error(`IV mismatch: possible replay attack or out-of-order message ` +
 ...
 */
export function wireToEnvelope(wire) {
    if (wire.length < 28) {
