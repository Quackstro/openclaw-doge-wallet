...
export class ConsumerSettlement {
    ...
    constructor(config) {
        this.config = config;
        this.htlcManager = new HTLCConsumerManager(this.storage, config.consumerPubkey, config.consumerPrivkey, config.consumerAddress);
    }
    /**
    ...
    async settle(params) {
        ...
        const fundedWire = sessionManager.buildResponse(offerMsg.id, fundedMsg);
...
export class ProviderSettlement {
    ...
    constructor(config) {
        this.config = config;
        this.htlcManager = new HTLCProviderManager(this.storage, config.providerPubkey, config.providerPrivkey, config.providerAddress);
    }
    /**
    ...
    async createAndSendOffer(params) {
        ...
        const { wire, messageId } = sessionManager.buildRequest(offerMsg);
    ...
    async waitForFundingAndClaim(params) {
        ...
        const claimWire = sessionManager.buildResponse(fundedMsg.id, claimMsg);
...
//# sourceMappingURL=htlc-settlement.js.map