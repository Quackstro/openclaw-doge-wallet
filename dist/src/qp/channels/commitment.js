...
export function buildCooperativeCloseTx(params, funding, state, consumerAddress, providerAddress, feeKoinu = DEFAULT_CLOSE_FEE_KOINU) {
    ...
    if (totalBalance > funding.depositKoinu) {
        throw new Error('Balance conservation violated: sum of balances exceeds deposit');
    }
    ...
    if (totalBalance === 0) {
        ...
        consumerPays = Math.min(Math.floor(feeKoinu * state.consumerBalance / totalBalance), state.consumerBalance);
