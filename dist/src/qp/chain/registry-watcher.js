...
import { REGISTRY_ADDRESSES } from '../registry.js';
import { scanAddress } from './scanner.js';
/**
 * Convert a SERVICE_ADVERTISE on-chain message to a ServiceListing
 */
function toServiceListing(msg) {
    if (msg.message.type !== QPMessageType.SERVICE_ADVERTISE)
        return null;
...
export class ServiceDirectory {
    ...
    getActive(currentBlock) {
        return Array.from(this.listings.values()).filter(l => {
            if (!l.expiresAtBlock)
                return true; // No expiry info — keep
...
export class RegistryWatcher {
    ...
    async scan() {
        ...
        for (const category of this.options.categories) {
            const address = REGISTRY_ADDRESSES[category];
            if (!address)
                continue;
            const fromBlock = this.state.lastScannedBlock[category] ?? 0;
            const messages = await scanAddress(this.provider, address, this.options.txLimit, {
            ...
            for (const msg of messages) {
                ...
                if (this.directory.get(msg.txid))
                    continue;
    ...
    async scanRevocations() {
        ...
        for (const category of this.options.categories) {
            ...
            for (const msg of messages) {
                ...
                if (this.processedRevocations.has(msg.txid))
                    continue;
...
//# sourceMappingURL=registry-watcher.js.map