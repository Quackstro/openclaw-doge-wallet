 ...
 */
export function computeTrustScore(metrics, weights = DEFAULT_WEIGHTS, caps = DEFAULT_CAPS) {
    ...
    const raw = (weights.rating * ratingNorm +
 ...
 */
export function determineTier(score, metrics) {
    const tiers = ['elite', 'trusted', 'established', 'emerging', 'new'];
    for (const tier of tiers) {
        const req = TIER_DEFINITIONS[tier];
        if (score < req.minScore)
            continue;
        if (score > req.maxScore)
            continue;
        // Check hard requirements
        if (req.minRatings && metrics.totalRatings < req.minRatings)
            continue;
        if (req.minUniqueClients && metrics.uniqueClients < req.minUniqueClients)
            continue;
        if (req.minAvgRating && metrics.averageRating < req.minAvgRating)
            continue;
        return tier;
    }
    return 'new';
 ...
 */
export function determineEffectiveTier(score, metrics) {
    // Walk down from highest tier
    const orderedTiers = ['elite', 'trusted', 'established', 'emerging', 'new'];
    for (const tier of orderedTiers) {
        const req = TIER_DEFINITIONS[tier];
        if (score < req.minScore)
            continue;
        // Check hard requirements
        const meetsRequirements = (!req.minRatings || metrics.totalRatings >= req.minRatings) &&
 ...
 */
export function computeReputation(metrics, weights, caps) {
 ...
 */
export function meetsMinPaymentThreshold(paymentKoinu) {
 ...
 */
export function paymentWeightedRating(ratings) {
