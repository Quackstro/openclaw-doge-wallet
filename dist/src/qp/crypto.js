 ...
 */
export function ecdhSharedSecret(privateKey, publicKey) {
 ...
 */
export function hkdfDerive(ikm, salt, info, length) {
 ...
 */
export function deriveIv(nonce, timestamp) {
 ...
 */
export function deriveHandshakeKey(sharedSecret, nonce) {
    return hkdfDerive(sharedSecret, nonce, 'qp-handshake-v1', 32);
}
/**
 * Derive session key from ephemeral ECDH (§7.4)
 */
export function deriveSessionKey(sessionSecret, sessionId) {
 ...
 */
export function randomNonce(size = 4) {
 ...
 */
export function decompressPublicKey(publicKey) {
    if (publicKey.length === 65) {
