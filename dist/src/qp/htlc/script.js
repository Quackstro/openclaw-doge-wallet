 ...
 */
export function buildRedeemScript(params) {
 ...
 */
export function createHTLC(params) {
 ...
 */
export function buildClaimScriptSig(signature, secret, redeemScript) {
 ...
 */
export function buildRefundScriptSig(signature, redeemScript) {
    const parts = [];
    // Push signature
    if (signature.length <= 75) {
        parts.push(Buffer.from([signature.length]));
 ...
 */
export function parseRedeemScript(script) {
    if (script.length !== 103) {
