...
export function buildFundingTransaction(params) {
    const { htlc, amountKoinu, feeBufferKoinu, sessionId, skillCode, consumerPubkey, utxos, changeAddress, feeKoinu, } = params;
...
export function buildClaimTransaction(params) {
    const { fundingTxId, fundingOutputIndex, secret, redeemScript, providerPrivkey, providerAddress, htlcAmountKoinu, feeKoinu, } = params;
    // Create transaction
    const tx = new Transaction();
    // Add HTLC input
    ...
    // Get signature
    const sighash = Transaction.Sighash.sign(tx, privKey, Transaction.Signature.SIGHASH_ALL, 0, Script.fromBuffer(redeemScript), htlcAmountKoinu);
...
export function buildRefundTransaction(params) {
    const { fundingTxId, fundingOutputIndex, redeemScript, consumerPrivkey, consumerAddress, htlcAmountKoinu, feeKoinu, timeoutBlock, } = params;
 ...
 */
export function estimateFee(txSizeBytes, feePerKb = 100_000_000) {
