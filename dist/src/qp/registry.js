 ...
 */
export function verifyRegistryAddresses() {
 ...
 */
export function getRegistryAddress(category) {
    return REGISTRY_ADDRESSES[category];
}
/**
 * Check if an address is a known registry address
 */
export function isRegistryAddress(address) {
...
export function decodeAddress(address) {
    ...
    if (decoded.length !== 21) {
        throw new Error(`Invalid address payload length: expected 21, got ${decoded.length}`);
    }
    return {
        version: decoded[0],
 ...
 */
export function encodeP2PKHAddress(hash) {
    if (hash.length !== 20) {
 ...
 */
export function encodeP2SHAddress(scriptHash) {
