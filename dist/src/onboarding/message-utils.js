 ...
 */
export function formatRecoveryPhrase(mnemonic) {
 ...
 */
export function verifyRecoveryWords(mnemonic, indices, answers) {
    const words = mnemonic.trim().split(/\s+/);
 ...
 */
export function inlineRow(...buttons) {
    return [buttons.map(b => ({ text: b.text, callback_data: b.callback }))];
}
/**
 * Create a multi-row inline keyboard.
 */
export function inlineGrid(rows) {
    return rows.map(row => row.map(b => ({ text: b.text, callback_data: b.callback })));
}
// ============================================================================
// Message Templates
// ============================================================================
...
export function welcomeMessage() {
    const text = '🐕 Welcome to DOGE Wallet!\n\n' +
    ...
    const keyboard = inlineRow({ text: '🚀 Let\'s Go', callback: 'doge:onboard:start' }, { text: '❓ Learn More', callback: 'doge:onboard:learn' });
    return { text, keyboard };
}
/**
 * Create the "learn more" expanded message.
 */
export function learnMoreMessage() {
    const text = '🐕 Welcome to DOGE Wallet!\n\n' +
        'I can help you send and receive Dogecoin right from this chat.\n\n' +
        '━━━━━━━━━━━━━━━━━━━━\n' +
        '📚 How It Works:\n\n' +
        ...
        '━━━━━━━━━━━━━━━━━━━━\n\n' +
        'Ready to set up your wallet?';
    const keyboard = inlineRow({ text: '🚀 Let\'s Go', callback: 'doge:onboard:start' });
    return { text, keyboard };
}
/**
 * Create the passphrase prompt message.
 */
export function passphrasePromptMessage() {
    return ('🔐 Step 1 of 4: Create Your Passphrase\n\n' +
 ...
 */
export function weakPassphraseMessage(issues) {
    const issueList = issues.map(i => `• ${i}`).join('\n');
    const text = '⚠️ Passphrase Strength Warning\n\n' +
    ...
    const keyboard = inlineRow({ text: '🔄 Try Again', callback: 'doge:onboard:passphrase_retry' }, { text: '⚡ Use Anyway', callback: 'doge:onboard:passphrase_weak_ok' });
    return { text, keyboard };
}
/**
 * Create the recovery phrase display message.
 */
export function recoveryPhraseMessage(formattedPhrase) {
    const text = '📝 Step 2 of 4: Your Recovery Phrase\n\n' +
    ...
    const keyboard = inlineRow({ text: '✅ I\'ve Written It Down', callback: 'doge:onboard:phrase_saved' });
    return { text, keyboard };
}
/**
 * Create the verification prompt message.
 */
export function verificationPromptMessage(indices) {
    return ('🔍 Step 3 of 4: Let\'s Verify\n\n' +
 ...
 */
export function verificationFailedMessage(mismatches) {
    ...
    const keyboard = inlineRow({ text: '🔄 Show Phrase Again', callback: 'doge:onboard:phrase_show_again' }, { text: '✏️ Try Again', callback: 'doge:onboard:verify_retry' });
    return { text, keyboard };
}
/**
 * Create the spending limits prompt message.
 */
export function limitsPromptMessage() {
    const text = '⚙️ Step 4 of 4: Spending Limits\n\n' +
 ...
 */
export function customLimitsMessage() {
    const text = '⚙️ Custom Spending Limit\n\n' +
 ...
 */
export function completionMessage(address) {
    const text = '🎉 Your DOGE Wallet is Ready!\n\n' +
    ...
    const keyboard = inlineRow({ text: '📊 View Dashboard', callback: 'doge:dashboard' });
    return { text, keyboard };
}
/**
 * Create the resume prompt for abandoned sessions.
 */
export function resumePromptMessage() {
    const text = '👋 Looks like you started setting up but didn\'t finish.\n\n' +
        'Would you like to continue where you left off?';
    const keyboard = inlineRow({ text: '▶️ Resume Setup', callback: 'doge:onboard:resume' }, { text: '🔄 Start Over', callback: 'doge:onboard:restart' });
    return { text, keyboard };
}
