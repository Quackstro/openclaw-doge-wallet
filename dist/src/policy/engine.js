...
export class PolicyEngine {
     ...
     */
    evaluate(amountDoge, recipient, _reason) {
     ...
     */
    determineTier(amountDoge) {
        const { tiers } = this.config;
        if (amountDoge <= (tiers.micro.maxAmount ?? 0))
            return "micro";
        if (amountDoge <= (tiers.small.maxAmount ?? 0))
            return "small";
        if (amountDoge <= (tiers.medium.maxAmount ?? 0))
            return "medium";
        if (amountDoge <= (tiers.large.maxAmount ?? 0))
            return "large";
        return "sweep";
    }
    /**
        ...
        switch (tierConfig.approval) {
            ...
            case "notify-delay":
                return {
                    ...
                    tier,
                    action: "delay",
                    reason: `Tier: ${tier} (${amountDoge} DOGE) — notification sent. ` +
