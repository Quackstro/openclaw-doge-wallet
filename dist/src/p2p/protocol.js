 ...
 *   [n] payload
 */
export function buildMessage(command, payload, network) {
 ...
 */
export function parseMessageHeader(data, network) {
    if (data.length < HEADER_SIZE)
        return null;
    const magic = data.readUInt32LE(0);
    if (magic !== MAGIC_BYTES[network])
        return null;
...
export function encodeVarStr(str) {
 ...
 */
export function serializeNetAddr(services, ip, port) {
 ...
 *   bool        relay
 */
export function buildVersionPayload(peerIp, peerPort, startHeight = 0) {
    const parts = [];
...
export function buildVerackMessage(network) {
 ...
 */
export function buildTxMessage(signedTxHex, network) {
