...
import { buildMessage, buildVersionPayload, buildVerackMessage, buildTxMessage, parseMessageHeader, HEADER_SIZE, } from "./protocol.js";
// ============================================================================
// Timeouts
// ============================================================================
 ...
 */
export async function broadcastViaP2P(signedTxHex, network, log) {
    const noop = () => { };
    const _log = log ?? noop;
    ...
    const results = await Promise.allSettled(peersToTry.map((peer) => relayToPeer(peer, signedTxHex, network, _log)));
    ...
    _log(success ? "info" : "warn", `doge-wallet: p2p: broadcast complete — ${peersReached}/${peersToTry.length} peers reached` +
 ...
 */
async function relayToPeer(peer, signedTxHex, network, log) {
