 ...
 */
export async function discoverPeers(network, maxPeers = 8, log) {
    ...
    const results = await Promise.allSettled(seeds.map((seed) => resolveSeed(seed, log)));
    // Collect unique IPs
 ...
 */
async function resolveSeed(seed, log) {
    try {
