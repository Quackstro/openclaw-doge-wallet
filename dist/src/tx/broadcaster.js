...
function isAlreadyBroadcast(error) {
    const lower = error.toLowerCase();
    return (lower.includes("already known") ||
...
function isDoubleSpend(error) {
    const lower = error.toLowerCase();
    return (lower.includes("double spend") ||
...
function isFeeTooLow(error) {
    const lower = error.toLowerCase();
    return (lower.includes("fee") && lower.includes("low") ||
 ...
 */
export async function broadcastTransaction(signedTxHex, provider, options = {}) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (isDoubleSpend(errMsg)) {
                throw new WalletError("DOUBLE_SPEND", `Double spend detected — inputs already spent: ${errMsg}`);
            }
            if (isFeeTooLow(errMsg)) {
                throw new WalletError("FEE_TOO_LOW", `Transaction fee too low for network acceptance: ${errMsg}`);
            }
            log("warn", `doge-wallet: broadcast attempt ${attempt}/${maxRetries} failed: ${errMsg}`);
    ...
    throw new WalletError("BROADCAST_FAILED", `Transaction broadcast failed after ${maxRetries} attempts (+ Blockchair fallback): ${lastError?.message ?? "unknown error"}. Much sadness. 🐕`);
}
/**
 ...
 */
export async function verifyBroadcast(txid, provider, timeoutMs = 30_000, pollIntervalMs = 5_000) {
 ...
 */
async function broadcastViaBlockchair(signedTxHex) {
 ...
 */
function computeTxid(signedTxHex) {
